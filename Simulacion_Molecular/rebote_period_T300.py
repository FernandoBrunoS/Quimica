import numpy as np
import matplotlib.pyplot as plt
from matplotlib.animation import FuncAnimation
from matplotlib import rc,rcParams

rc('animation',html='jshtml')
rcParams['animation.embed_limit'] = 2**128

kb=3.16E-6

N = 5 # no de partículas
L = 10 # lado

pos = np.random.uniform(0,L,size=(N,2))

vels = np.random.rand(N,2)-0.5

################################
# Cálculo de la Temperatura

magv = np.linalg.norm(vels,axis=1)
s=10
magv2 = magv**2

sumav2 = magv2.sum()

Tini = sumav2/(2*kb*N)

T = 300 # Set temperature to 300K
#### Ajuste de Temperaturas
f = np.sqrt(T/Tini)  # factor de ajuste

nvels = f*vels # Velocidades ajustadas

##################################
# Obtener un color para cada partícula
cmap = plt.get_cmap('Spectral',N)

################################1000
# Función que hace que reboten con las paredes

''' Propiedades periódicas a la frontera
'''

def pperiod(pos,N,L):
  for i in range(N):
     if pos[i][0]>L:
       pos[i][0]-=L
     elif pos[i][0]<0:
       pos[i][0]+=L

     if pos[i][1]>L:
       pos[i][1]-=L
     elif pos[i][1]<0:
       pos[i][1]+=L
  return pos

def circulos(ax,pos,r):
   for i in range(N):
       circ = plt.Circle((pos[i][0],pos[i][1]),r,color=cmap(i),ec='k',clip_on=True)
       ax.add_patch(circ)

'''
Vamos a crear una función que identifique cuándo chocan las pelotas y
'''
def choque(pos,nvels,r,N):
    for i in range(N):
        for j in range(i+1,N):
           d = np.sqrt((pos[j][0]-pos[i][0])**2+(pos[j][1]-pos[i][1])**2)
           if d<=2*r:
              tempvix = nvels[i][0]
              tempviy = nvels[i][1]

              nvels[i][0]=nvels[j][0]
              nvels[i][1]=nvels[j][1]

              nvels[j][0]=tempvix
              nvels[j][1]=tempviy

              del tempvix
              del tempviy

    return nvels



#################################
# Changed to a single plot for animation
fig,ax = plt.subplots(figsize=(3,3))

npasos = 1000000 # Reduced npasos for faster GIF generation (original was 20,000,000)

t = 0
dt = 0.0001 # dt as requested
G = []
conteo = 0
r = 0.5
for i in range(npasos):
   t += dt # t = t + dt
   pos = pos + nvels*dt # Este paso es la integración numérica
   choque(pos,nvels,r,N) # Para ver si chocan entre sí
   #pared_r(pos,nvels,N,L,r)
   pperiod(pos,N,L)
   if i%(npasos/10000)==0: # Save frames less frequently for very long simulations
      G.append(pos.copy()) # Use .copy() to avoid issues with references
   if i%(npasos/10)==0:
      conteo += 1
      print('>'*conteo+'-'*(10-conteo))

def pelicula(i):
    # Reverted to single plot animation logic
    ax.clear()
    ax.set_aspect(1)
    ax.axis('off')
    plt.xlim(0,L) # ax.set_xlim(0,L)
    plt.ylim(0,L) # ax.set_ylim(0,L)
    circulos(ax,G[i],r=0.5)
    #plt.scatter(G[i][:,0],G[i][:,1],s=100,c=range(N),cmap='rainbow',ec='k')

anim = FuncAnimation(fig,pelicula,frames=range(len(G)))
anim.save('rebote_period_T300.gif', writer='pillow', fps=50)
plt.close(fig) # Close the figure to prevent it from displaying a static plot
